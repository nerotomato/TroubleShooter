 ...
 * 需要实现一个全局过滤器AuthGlobalFilter，当鉴权通过后将JWT令牌中的用户信息解析出来，然后存入请求的Header中
 * 这样后续服务就不需要解析JWT令牌了，可以直接从请求的Header中获取到用户信息
 *
 * @author nerotomato
 * @create 2021/8/3 下午5:02
 */
@Slf4j
@Component
public class AuthGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        try {
            //token中解析用户信息并设置到Header中
            String replacedToken = token.replace("Bearer", "");
            JWSObject jwsObject = JWSObject.parse(replacedToken);
